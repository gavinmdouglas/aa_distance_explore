...
all_asymmetric_aa_combos <- table(c(subtab_filt_asymmetric_buried$aa_combo, subtab_filt_asymmetric_exposed$aa_combo))
ubiq_asymmetric_aa_combos <- names(all_asymmetric_aa_combos)[which(all_asymmetric_aa_combos == 2)]
...
for (AF_var in AF_vars) {
  raw <- lapply(metrics, function(metric) {
    ...
    if (metric == "RSA_only") {
      ...
      form <- as.formula(paste0(AF_var, "_orderedNorm ~ mainly_mut_type"))
    } else if (metric == "no_metric") {
      form <- as.formula(paste0(AF_var, "_orderedNorm ~ RSA_group + mainly_mut_type"))
    } else {
      ...
      form <- as.formula(paste0(AF_var, "_orderedNorm ~ RSA_group + mainly_mut_type + ", metric_name))
    }
    ...
    if (metric == "RSA_only") {
      ...
      nonCpG_transition_coef <- NA
      CpG_transition_coef <- NA
    } else if (metric == "mut_only") {
      metric_coef <- "NA"
      RSA_group_coef <- "NA"
      nonCpG_transition_coef <- cf["mainly_mut_typeNon-CpG transition"]
      CpG_transition_coef <- cf["mainly_mut_typeCpG transition"]
    } else if (metric == "no_metric") {
      metric_coef <- "NA"
      RSA_group_coef <- cf["RSA_groupexposed"]
      nonCpG_transition_coef <- cf["mainly_mut_typeNon-CpG transition"]
    ...
    data.frame(AF_var = AF_var,
               ...
               nonCpG_transition = nonCpG_transition_coef,
               CpG_transition = CpG_transition_coef,
...
write.table(x=results,
            file=gzfile('~/Drive/research/aa_distance/aa_distance_zenodo/allele_freq_vs_predicted_effects/human_variants/human_pref_vs_AF_model_results.tsv.gz'),
