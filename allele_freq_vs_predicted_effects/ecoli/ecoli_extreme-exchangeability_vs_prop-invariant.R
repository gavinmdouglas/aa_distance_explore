...
exchange_tab <- read.table('/Users/gavin/Drive/ncsu/aa_selection/aa_selection_zenodo/ecoli_variants/per_codon_exchangeability_invariant_vs_freq.tsv.gz',
...
for (pref in prefs) {
  print(pref)
  print(mean(exchange_tab[which(exchange_tab$Class == 'Invariant'), pref]))
  print(mean(exchange_tab[which(exchange_tab$Class == 'atleast0.01'), pref]))
...
combined_prop_invariant$background_overall_rare_prop <- length(which(exchange_tab$Class == 'Invariant')) / nrow(exchange_tab)

write.table(combined_prop_invariant,
            file = gzfile('~/Drive/ncsu/aa_selection/aa_selection_zenodo/ecoli_variants/sites_extreme_exchangeabilities_prop_invariant.tsv.gz'),
