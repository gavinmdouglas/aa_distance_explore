...
dms_folder = "/Users/gavin/Drive/research/aa_distance/data/DMS_ProteinGym_substitutions/"
...
for dms_file in dms_files:
    ...
    with open(os.path.join(dms_folder, dms_file), "r") as dms_fh:
        ...
        if num_sites_tested >= 20 and prop_sites_tested >= 0.95 and np.mean(site_num_aa_tested) >= 15:
            ...
            print(dms_file, str(min_dms_score), str(max_dms_score), str(num_sites_tested), str(prop_sites_tested), str(np.mean(site_num_aa_tested)))