...
dist_file_to_consistent_simfile <- function(dist_file) {
  ...
  intab$aa1 <- rownames(intab)
  out_tab <- reshape2::melt(intab, id.vars='aa1')
  colnames(out_tab) <- c('aa1', 'aa2', 'similarity')
  out_tab <- out_tab[out_tab$aa1 != out_tab$aa2, ]
  out_tab$similarity <- -1 * out_tab$similarity
  out_tab$similarity <- transform_to_range(out_tab$similarity)
  
  outfile <- paste0('~/Drive/research/aa_distance/aa_distance_zenodo/aa_metrics/prepped_similarity_consistent/', filebase, '.tsv.gz')
...
unique(sapply(dist_files, dist_file_to_consistent_simfile))
...
unique(sapply(names(sim_raw),
              function(x) { 
                ...
                if (nrow(intab) != 20 || ncol(intab) != 20) { 
                  message(x)
                  stop('Input table does not have 20 rows and 20 columns!')
                }
                ...
                intab$aa2 <- rownames(intab)
                out_tab <- reshape2::melt(intab, id.vars='aa2')
                colnames(out_tab) <- c('aa2', 'aa1', 'similarity')
                out_tab <- out_tab[, c('aa1', 'aa2', 'similarity')]
                out_tab <- out_tab[which(out_tab$aa1 != out_tab$aa2), ]
                out_tab$similarity <- transform_to_range(out_tab$similarity)
                
                outfile <- paste0('~/Drive/research/aa_distance/aa_distance_zenodo/aa_metrics/prepped_similarity_consistent/', x, '.tsv.gz')
...
write.table(x = blosum62_prob_long,
                   file = gzfile('~/Drive/research/aa_distance/aa_distance_zenodo/aa_metrics/prepped_similarity_consistent/blosum62.prob.tsv.gz'),
                   quote = FALSE, sep = '\t', col.names = TRUE, row.names = FALSE)
...
write.table(x = vtml200_prob_long,
                   file = gzfile('~/Drive/research/aa_distance/aa_distance_zenodo/aa_metrics/prepped_similarity_consistent/vtml200.prob.tsv.gz'),
                   quote = FALSE, sep = '\t', col.names = TRUE, row.names = FALSE)
