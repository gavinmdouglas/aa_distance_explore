...
def main():
    ...
    with open(args.input, 'r') as input_fh:
        ...
        if args.header:
            print("subsample_num\tlnL\tnum_param\ttree_length\tkappa\tomega\ttree_length_dN\ttree_length_dS")

        print(f"{subsample_num}\t{lnL_hits[0]}\t{np_hits[0]}\t{tree_length_hits[0]}\t{kappa_hits[0]}\t{omega_hits[0]}\t{tree_length_dN[0]}\t{tree_length_dS[0]}")
